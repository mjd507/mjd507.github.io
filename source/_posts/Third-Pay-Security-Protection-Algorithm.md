...
百度钱包目前只有 H5 收营台，后台直接采用分配给商家的 key 通过 MD5 加密请求参数来验证签名。
...
翼支付 App 收营台分两步，下单，组装支付参数。下单验签与 H5 一致，唤起收营台交由集成的 SDK 处理。

### 付款码 收银台

商家扫描用户二维码，向翼支付系统下单，客户端轮询查看支付结果。其中下单验签与 H5 一致，支付扣款翼支付内部处理。

## 支付宝

支付宝 APP，H5，BarCode 都采用 RSA 非对称加密，最新接入的采用 RSA2 加密。
...
非对称解密对机器有一定的性能消耗，尤其在请求参数很多很长的情况下，个人感觉处理的不如翼支付 H5 版的好。
